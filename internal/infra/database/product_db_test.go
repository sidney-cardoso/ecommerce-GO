...
func TestFindProductByID(t *testing.T) {
	...
	product, err = produtDB.FindById(product.ID.String())
	assert.NoError(t, err)
...
func TestDeleteProduct(t *testing.T) {
	...
	db.Create(product)
	produtDB := NewProduct(db)
	err = produtDB.Delete(product.ID.String())
	assert.NoError(t, err)
	_, err = produtDB.FindById(product.ID.String())
	assert.Error(t, err)
}
